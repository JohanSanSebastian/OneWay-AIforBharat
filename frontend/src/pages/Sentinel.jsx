...
export default function Sentinel() {
  ...
  const handleParkingAssist = async () => {
    if (!parkingPhoto) return
    setIsLoading(true)
    try {
      const { data } = await sentinelApi.parkingAssist(parkingPlate || '', parkingPhoto)
        ...
        <div className="max-w-xl animate-fade-in-up">
          <div className="bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl p-6 card-lift">
            ...
            <p className="text-slate-400 text-sm mb-4">
              Upload a photo of the obstructing vehicle. We will notify the owner automatically.
            </p>
            ...
            <div className="flex gap-3 items-center">
              <div className="flex-1">
                <input
                  ...
                  className="w-full file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-white/5 file:text-white file:cursor-pointer text-slate-400"
                />
              </div>
              <button
                onClick={handleParkingAssist}
                disabled={!parkingPhoto || isLoading}
