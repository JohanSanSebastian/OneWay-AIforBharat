...
async def parking_assist(
    plate_number: str = Form(...),
    photo: UploadFile = File(...)
):
    """
    Trigger parking obstruction resolution with photo upload.
    """
    # Save uploaded photo
    image_id = str(uuid.uuid4())
    image_path = VIOLATIONS_DIR / f"parking_{image_id}.jpg"
    with open(image_path, "wb") as f:
        f.write(await photo.read())

    # Trigger WhatsApp/call (hardcoded number)
    engine = get_sentinel_engine()
    call_result = await engine.trigger_parking_call(
        plate_number,
        "8943377737"  # always use hardcoded number
    )
